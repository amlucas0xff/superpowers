---
name: plan-to-epic
description: Convert a superpowers implementation plan into a beads epic with properly structured tasks, acceptance criteria, and inferred dependencies. Requires beads (bd) to be installed.
---
...
if [ -d ".beads" ] && command -v bd &> /dev/null; then
  ...
  echo "ERROR: Beads not active. Run 'bd init' or install beads first."
  exit 1
fi
```
...
**Description (--description):** Implementation-focused content - what to do:
- Task summary
- Files to modify
...
- Verification steps
- Test descriptions
- Standard criteria (build passes, tests pass, code committed)
